...
(function () {
  ...
  var CFG = {
    ...
    keys: {
      cart: 'vecchia.cart.v2',
      wish: 'vecchia.wishlist.v2',
      seen: 'vecchia.seen.v1',
      notice: 'vecchia.notice.v1'
    },
    legacy: { cart: 'vecchia_cart', wish: 'vecchia_wishlist' },

    /* ── BARRA DE AVISO ──────────────────────────────────────────────
       El mensaje de la franja superior. Cambia el texto y listo.
       Pon  text: ''  para que no aparezca.
       Si cambias el mensaje, sube también el número de `version`: así
       vuelve a mostrarse a quien ya la había cerrado.                */
    notice: {
    ...
    /* ── DESCUENTO POR CANTIDAD ──────────────────────────────────────
       Desactivado. Para activarlo pon las dos cifras, por ejemplo
       minQty: 2 y percent: 10  ->  «2 o más, 10% de descuento».
       Con percent en 0 no se aplica ni se muestra nada.              */
  ...
  /* Enlace absoluto a la ficha de un producto, para poder compartirlo */
  function productUrl(id) {
    var base = location.href.replace(/[^\/]*$/, '');
    return base + 'producto.html?id=' + encodeURIComponent(id);
  }

  /* Mensaje para enviar una fragancia a otra persona por WhatsApp.
     Sin número de destino: lo elige quien comparte. */
  function shareLink(p) {
    var NL = String.fromCharCode(10);
    var out = [p.name];
    if (p.brand) out.push(p.brand);
    if (p.price) {
      var bs = Rate.bs(toCents(p.price));
      out.push(money(toCents(p.price)) + (bs ? ' \u00b7 ' + bs : ''));
    }
    out.push(p.soldOut ? 'Sin stock' : 'Disponible');
    out.push('');
    out.push(productUrl(p.id));
    out.push('');
    out.push('VECCHIA PERFUMES');
    return 'https://wa.me/?text=' + encodeURIComponent(out.join(NL));
  }
  ...
  var Cart = {
    ...
    /* Descuento por cantidad, en céntimos enteros y redondeado una sola
       vez sobre el subtotal: así total = subtotal - descuento siempre
       cuadra al céntimo. Si está desactivado devuelve 0. */
    discountCents: function () {
      var d = CFG.discount;
      if (!d || !(d.percent > 0) || !(d.minQty > 0)) return 0;
      if (Cart.count() < d.minQty) return 0;
      return Math.round(Cart.subtotalCents() * d.percent / 100);
    },
    discountLabel: function () {
      var d = CFG.discount;
      if (!Cart.discountCents()) return '';
      return 'Descuento ' + d.percent + '% (' + d.minQty + ' o más)';
    },
    totalCents: function () {
      return Cart.subtotalCents() - Cart.discountCents();
    },
    add: function (id, qty) {
      var p = BY_ID[id];
      if (!p) return false;
  ...
  /* ── Vistos recientemente ───────────────────────────────────────
     Se guardan solo los identificadores, en el navegador del visitante.
     Nada sale del dispositivo. */
  var Seen = {
    MAX: 8,
    list: function () {
      var raw = Store.read(CFG.keys.seen, []);
      if (!Array.isArray(raw)) return [];
      return raw.filter(function (id) { return BY_ID[id]; });
    },
    add: function (id) {
      if (!BY_ID[id]) return;
      var l = Seen.list().filter(function (x) { return x !== id; });
      l.unshift(id);
      Store.write(CFG.keys.seen, l.slice(0, Seen.MAX));
    },
    /* Los productos vistos, sin contar el que se está mirando ahora */
    products: function (exceptId) {
      return Seen.list()
  ...
  /* Franja superior con el aviso del momento. Se puede cerrar y no
     vuelve a salir hasta que la tienda cambie `notice.version`. */
  function buildNotice() {
    var n = CFG.notice;
    if (!n || !n.text) return;
    if (Store.read(CFG.keys.notice, 0) === n.version) return;

    var bar = el('div', { class: 'notice', id: 'v-notice', role: 'status' }, '' +
      '<div class="notice__inner">' +
        '<p>' + esc(n.text) + '</p>' +
        '<button class="notice__x" id="v-notice-x" aria-label="Cerrar aviso">' +
          ICON.close + '</button>' +
      '</div>');
    ...
    document.documentElement.classList.add('has-notice');

    function measure() {
      document.documentElement.style.setProperty(
        '--notice-h', Math.round(bar.offsetHeight) + 'px');
    }
    measure();
    window.addEventListener('resize', debounce(measure, 120));
    if (window.ResizeObserver) new ResizeObserver(measure).observe(bar);

    $('#v-notice-x').addEventListener('click', function () {
      Store.write(CFG.keys.notice, n.version);
      bar.remove();
        ...
        '<div class="mobile-nav__inner">' +
          ...
          '<div class="mobile-nav__brands">' +
            BRANDS.slice(0, 8).map(function (b) {
              return '<a class="chip" href="marca.html?m=' + encodeURIComponent(b.name) + '">' +
                esc(b.name) + ' <span>' + b.n + '</span></a>';
            }).join('') +
            '<a class="chip" href="marca.html">Ver todas</a>' +
          '</div>' +
        '</div>' +
      ...
      '<div class="wrap">' +
        '<div class="footer__grid">' +
          ...
          '<div><h3>Ayuda</h3><ul>' +
            ...
            '<li><a href="marca.html">Todas las marcas</a></li>' +
          '</ul></div>' +
  ...
  var Drawer = {
    ...
    render: function () {
      ...
      if (isCart) {
        ...
        var desc = Cart.discountCents();
        foot.innerHTML =
          '<div class="totals">' +
            '<div class="t-sub"><span>Subtotal (' + Cart.count() + ' art.)</span><span>' + money(sub) + '</span></div>' +
            (desc ? '<div class="t-sub t-desc"><span>' + esc(Cart.discountLabel()) +
                    '</span><span>\u2212 ' + money(desc) + '</span></div>' : '') +
  ...
  window.Vecchia = {
    ...
    Rate: Rate, paintBs: paintBs, Seen: Seen,
    shareLink: shareLink, productUrl: productUrl,
